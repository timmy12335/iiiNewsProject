...
public class SMSHttp {
	...
	/// <param name="mobile">接收人之手機號碼。格式為:
	/// +886912345678或09123456789。多筆接收人時，請以半形逗點隔開( ,
	/// )，如0912345678,0922333444。</param>
	/// <param
	/// name="sendTime">簡訊預定發送時間。-立即發送：請傳入空字串。-預約發送：請傳入預計發送時間，若傳送時間小於系統接單時間，將不予傳送。格式為YYYYMMDDhhmnss；例如:預約2009/01/31
	/// 15:30:00發送，則傳入20090131153000。若傳遞時間已逾現在之時間，將立即發送。</param>
	...
	public boolean sendSMS(String userID, String password, String subject, String content, String mobile,
		...
		try {
			...
			if (!resultString.startsWith("-")) {
				 ...
				 * CREDIT：發送後剩餘點數。負值代表發送失敗，系統無法處理該命令 SENDED：發送通數。 COST：本次發送扣除點數
				 * UNSEND：無額度時發送的通數，當該值大於0而剩餘點數等於0時表示有部份的簡訊因無額度而無法被發送。
				 * BATCH_ID：批次識別代碼。為一唯一識別碼，可藉由本識別碼查詢發送狀態。格式範例：220478cc-8506-49b2-93b7-
				 * 2505f651c12e
				 */

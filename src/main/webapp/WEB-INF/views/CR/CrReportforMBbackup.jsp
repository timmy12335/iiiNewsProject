...
<body>
	...
	<section style="margin-top: 100px;">
		...
		<h2 class="font-weight-bold mb-0" align="center"><b>${MBBean.name}${CpMemberBean.cpname} :</b>
			 客服表單列表</h2>
		<br>
		...
		<table class="table table-hover fontsize"
			...
			<thead class="table-primary" style="background-color: #6FB7B7;"
				...
				<tr>
					...
					<th width='160px;'>申請人</th>
					<th width='120px;'>類別</th>
					<th width='160px;'>標題</th>
					<th width='120px;'>內容</th>
			...
			<tbody style="background-color: #FCFCFC;">
				<c:choose>
					<c:when test="${empty CrReport}">
						<tr>
							<td colspan='12' style="text-align:center"><font color='red'>無客服資料</font></td>
						</tr>
					</c:when>
					<c:otherwise>
						<c:forEach var='ser' items='${CrReport}' varStatus="report">
							<tr style="height: 100px;">
								<td style='vertical-align: middle;text-align:center;'>${ser.pk}</td>
								...
								<td style='vertical-align: middle; cursor: pointer;'
									onclick="MBfolded(${report.index})" class="box"><p
										class="ellipsis" title='點擊顯示詳細內容'>${ser.crContent}</p></td>
							...
							<tr id="MBcontent${report.index}"
								...
								<td colspan="2"
									style='text-align: center; vertical-align: middle;'><b><font
										size="4">客服內容:</font></b></td>
								<td colspan="12"><div
										style="height: 80px; display: flex; align-items: center; text-align: center;">
										<font size='3'>${ser.crContent}</font>
									</div>
							<tr id="recontent${report.index}"
								style="display: none; word-wrap: break-word; background-color: #ECF5FF">
								<td colspan="2"
									style='text-align: center; vertical-align: middle;'><b><font
										size="4">回覆內容:</font></b></td>
								<td colspan="11" style='vertical-align: middle;'><div
										style="height: 80px; display: flex; align-items: center;">
										<font size='3'>${ser.crReContent}</font>
									</div></td>
						</c:forEach>
					</c:otherwise>
				</c:choose>
